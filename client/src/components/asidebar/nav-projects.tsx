        ...
        <SidebarMenu className="h-[320px] scrollbar overflow-y-auto pb-2">
          ...
          {!isPending && projects?.length === 0 ? (
            ...
            projects.map((item) => {
              ...
              return (
                <SidebarMenuItem key={item._id} className="p-1">
                  <SidebarMenuButton 
                    asChild 
                  ...
                  >
                    <Link to={projectUrl} className="!text-[15px] flex items-center gap-2">
                      <span className="w-5 h-5 flex items-center justify-center text-lg text-purple-500">{item.emoji}</span>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent
                      className="w-48 rounded-xl bg-white/95 backdrop-blur-md border border-purple-200/60 shadow-xl overflow-hidden py-2"
          ...
          {hasMore && (
            <SidebarMenuItem className="p-1">
              <SidebarMenuButton
                className="rounded-lg px-3 py-2 text-slate-700 hover:bg-purple-50/20 transition-colors duration-200"
                disabled={isFetching}
                onClick={fetchNextPage}
              >
                <MoreHorizontal className="w-5 h-5 text-purple-500" />
                <span className="font-medium">{isFetching ? "Loading..." : "More"}</span>
              </SidebarMenuButton>
            </SidebarMenuItem>
          )}
        </SidebarMenu>
